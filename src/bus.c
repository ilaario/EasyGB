...
bus bus_init(cartridge cart) {
    ...
    if (rbus->mem->boot_rom_loaded) {
        // Keep power-on state when the DMG boot ROM is mapped in.
        // The boot ROM itself initializes IO/registers and then unmasks cartridge ROM via FF50.
        io[0x50] = 0x00; // BOOT (enabled)
        dbg_log("BUS init: boot ROM active, using power-on IO state");
    } else {
        // Fallback without boot ROM: initialize to the usual post-boot DMG state.
        io[0x00] = 0xCF; // JOYP
        io[0x01] = 0x00; // SB
        io[0x02] = 0x7E; // SC
        io[0x04] = 0xAB; // DIV
        ...
        io[0x4A] = 0x00; // WY
        io[0x4B] = 0x00; // WX
        io[0x50] = 0x01; // BOOT (disabled)
        dbg_log("BUS init: no boot ROM, using post-boot IO state");
    }
